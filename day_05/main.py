...
# trying to use a test function from now on
def testFunc(testFile: str, realSolution: str, part: int) -> None:
    if part == 1:
    ...
    with open(inFile) as f:
        ...
        for idx, line in enumerate(f):
            if line[1] == '1':
                ...
                # replace the empty values with zeros, so later we can rotate the matrix by transposing it
                strLine = line.strip('\n').replace('    ', '0').replace(
                    '[', '').replace(']', '').replace(' ', '')
                data.append([*strLine])

        # here is the transposing and revrsing the order to have the upper crates at the end of the lists
